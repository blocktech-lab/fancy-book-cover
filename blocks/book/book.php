...
$book_link = BL_Blocks::getAttribute( $args, 'book_cover_link' );

// Define the “alt” text.
$img_alt_value = BL_Blocks::getImageField($args, 'book_cover_img', 'alt', true);
$img_alt = $img_alt_value ?? __( 'Cover image', 'fancy-book-cover' );

 // Get user color choices.
$back_cover_color = BL_Helper_v2::sanitizeRgba( BL_Blocks::getAttribute( $args, 'back_cover_color' )) ?? 'rgba(255,255,255,1)';
$back_cover_bkg_color = BL_Helper_v2::sanitizeRgba( BL_Blocks::getAttribute( $args, 'back_cover_bkg_color' )) ?? 'rgb(0,73,255,1)';
$back_cover_icon_type = BL_Blocks::getAttribute( $args, 'back_cover_icon_type' ) ?? 'download';
...
$attachment_id = BL_Blocks::getImageField( $args, 'book_cover_img', 'id', true );

// If a custom image was chosen.
if ( 0 !== $attachment_id ) :

    // User selected image size.
    $selected_size = BL_Blocks::getImageField( $args, 'book_cover_img', 'size', true );
    ...
    if ( isset( $img_src_array[0] ) ) {
        ...
        $selected_size_url = COVER_URL . '/assets/cover2x.png';
    }
    ...
    $medium_size_url = COVER_URL . '/assets/cover.png';
...
<div <?php echo ( empty( $context ) || $context !== 'edit' ) ? get_block_wrapper_attributes( [ 'class' => "book-cover" ] ) : 'data-wp-block-props="true"'; ?>>
    ...
    <div class="book-cover-wrapper">
        ...
        <a class="book-cover-link" href="<?php echo esc_url( BL_Blocks::getLinkUrl( $args, 'book_cover_link' ) ) ?>" id="<?php echo "book-cover-link-" . esc_attr( $block_id ) ?>" aria-labelledby="<?php echo "book-cover-label-" . esc_attr( $block_id ) ?>">
        <?php endif; ?>
            <div class="book-cover-container" id="<?php echo "book-cover-container-" . esc_attr( $block_id ) ?>">
                <div class="book-cover-image" data-icon="<?php echo esc_attr( BL_Blocks::getAttribute( $args, 'back_cover_icon_type' ) ) ?>" data-size="<?php echo esc_attr( BL_Blocks::getAttribute( $args, 'book_size' ) ) ?>">
                    ...
                    <div class="book-backcover" <?php echo 'small' === BL_Blocks::getAttribute( $args, 'book_size' ) ? 'aria-hidden="true"' : '' ?>>
                        <?php echo 'small' !== BL_Blocks::getAttribute( $args, 'book_size' ) ? esc_html( BL_Blocks::getAttribute( $args, 'back_cover_text' ) ) : '' ?>
                    </div>
                </div>
            </div>
        <?php if ( $book_link['url'] && empty( $_GET['context'] ) ) : // If a link was added. ?>
        </a>
        <?php echo '<span id="book-cover-label-' . esc_attr( $block_id ) . '" class="book-cover-sr-only">' . sprintf(__('%s', 'fancy-book-cover'), esc_html( $img_alt ) ) . '</span>';
        endif; ?>
    </div>
</div>