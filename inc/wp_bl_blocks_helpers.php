    ...
    class BL_Blocks
    {
        ...
        static function register_block_type($reg_args)
        {
            ...
            if (!self::$helpers_registered) {
                ...
                wp_register_script('bl-blocks-controls',
                    $base_url . 'blocks/bl-blocks-controls.js',
                    array('wp-blocks', 'wp-block-editor', 'wp-server-side-render', 'wp-media-utils', 'wp-data', 'wp-element', 'lodash'), $reg_args[ 'version' ]);

                wp_register_style('bl-blocks-controls-style',
                    $base_url . 'blocks/bl-blocks-controls.css',
            ...
            $editor_style_handle = 'bl-blocks-controls-style';

            if (!empty($reg_args['enqueue_editor_style'])) {
                $editor_style_handle = 'block-editor-' . md5($reg_args['enqueue_editor_style']);
                wp_register_style($editor_style_handle, $reg_args['enqueue_editor_style'], array('bl-blocks-controls-style'), $reg_args[ 'version' ]);
            }

            $script_dependencies = array('wp-blocks', 'wp-block-editor', 'wp-server-side-render', 'wp-media-utils', 'wp-data', 'wp-element', 'bl-blocks-controls');
            ...
            wp_localize_script($editor_script_handle,
                'bl_project_data_' . str_replace('-', '_', $slug),
                array(
                    'url' => $base_url,
                )
            );
        ...
        static function getImageUrl($args, $prop, $size, $use_default = true)
        {
            ...
            return BL_Image::getUrl($a[$prop]['id'], $size);
        }
